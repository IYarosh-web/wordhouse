...
export const deleteDefinitionFormSubmittedFx = createEffect((event: React.FormEvent<HTMLFormElement>) => {
...
export const deleteSentenceFormSubmittedFx = createEffect((event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
...
export const addDefinitionFormSubmittedFx = createEffect((event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
...
export const addSentenceFormSubmittedFx = createEffect((event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
...
sample({
    ...
    source: $wordStore,
    filter: (words, sentence) => words.some(word => word.id === sentence.wordId),
    fn: (words, sentence) => {
...
sample({
    ...
    source: $wordStore,
    filter: (words, definition) => words.some(word => word.id === definition.wordId),
    fn: (words, definition) => {
...
sample({
    ...
    fn: (words, data) => {
        ...
        word.definitions = word.definitions.filter(definition => definition.id !== data.definitionId);
        return word;
    },
    target: updateWord,
});
...
sample({
    ...
    fn: (words, data) => {
        const word = deepClone(words.find(word => word.id === data.wordId));
        word.sentences = word.sentences.filter(sentence => sentence.id !== data.sentenceId);
        return word;
    },
    target: updateWord,
});